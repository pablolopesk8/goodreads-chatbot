...
const {  } = require('../../src/services/messageTemplates.service');
...
describe('Message Templates Service Test', () => {
    ...
    describe('Books List Message', () => {
        it('Should be generated a Books List message with 5 items, all of them correctly', async () => {
            ...
            const message = BooksListMessage();
            ...
            for (let i = 0, len = message[1].attachment.payload.elements.length; i < len; i++) {
                ...
                item.should.have.property('title').and.be.equa(booksArray[i].title);
                item.should.have.property('subtitle').and.be.equa(booksArray[i].subTitle);
                item.should.have.property('image_url').and.be.equa(booksArray[i].imageUrl);
                item.should.have.property('buttons').and.be.a.Array();
                item.buttons.should.have.property('type').and.be.equal('postback');
                item.buttons.should.have.property('title').and.be.a.String();
                item.buttons.should.have.property('payload').and.be.equal(`CHOOSE_BOOK_${booksArray[i].id}`);
            }
        });
    });
    ...
    describe('Suggest Message', () => {
        it('Should be generated an affirmative Suggest message correctly', async () => {
            ...
            message.should.be.a.Array();
            message[0].should.have.property('text').and.be.a.String().and.match(/:\)/);
            ...
            message[1].quick_replies[0].should.have.property('content_type').and.be.equal('text');
            message[1].quick_replies[0].should.have.property('title');
            message[1].quick_replies[0].should.have.property('payload').and.be.equal('SEARCH_ANOTHER');
        });
        ...
        it('Should be generated a non-conclusive Suggest message correctly', async () => {
            ...
            message.should.be.a.Array();
            message[0].should.have.property('text').and.be.a.String().and.match(/¯\\_\(ツ\)_\/¯/);
